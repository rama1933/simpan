<template>
  <AdminLayout page-title="Edit User" :user="user">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
      <h1 class="text-2xl font-semibold text-gray-900 mb-4">Edit User</h1>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.password" type="password" class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Konfirmasi Password</label>
              <input v-model="form.password_confirmation" type="password" class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>
          <div>
...
function submit() {
  router.post(route('users.update', props.editing.id), { ...form, _method: 'PUT' }, {
    onSuccess: () => {},
  })
}
</script>
