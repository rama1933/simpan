      ...
      <div class="w-full mx-auto py-6 px-6">
        <div class="px-0">
          ...
          <div class="mb-6 flex items-center justify-between">
            <Link :href="route('admin.knowledge.show', knowledge.id)" class="text-brand-700 hover:text-brand-800 mr-4">
          ...
          <Form ref="formRef" :validation-schema="schema" @submit="onSubmit" v-slot="{ meta, values }" class="space-y-6">
            <div class="grid grid-cols-1 gap-6">
              <div class="space-y-6 bg-white shadow rounded-lg p-6">
                ...
                <div>
                  ...
                  >
                    <input ref="fileInput" type="file" multiple class="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png" @change="onFilesSelected" />
            ...
            <div class="sticky bottom-0 inset-x-0 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 border-t border-gray-200 px-6 py-4 flex justify-end gap-3 shadow-lg">
              <Link :href="route('admin.knowledge.show', knowledge.id)" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500">Batal</Link>
...
const props = defineProps<{ knowledge: any, categories: any[], skpds: any[], user: any }>()
...
const onFilesSelected = (e: Event) => {
  ...
  if (files.length) {
    ...
    const allowed = files.filter((f) => allowedTypes.includes(f.type))
...
const onSubmit = async (values: any) => {
  ...
  router.post(route('admin.knowledge.update', props.knowledge.id), fd, {
    forceFormData: true,
    onSuccess: () => {
